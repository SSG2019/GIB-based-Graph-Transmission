...
def cross_validation_with_val_set(dataset, model, discriminator, model_path, correct_p, commitment_cost, folds, epochs,
                                  batch_size,
    ...
    for fold, (train_idx, test_idx,
        ...
        for epoch in range(1, epochs + 1):
            train_loss = train(model, correct_p, commitment_cost, discriminator, optimizer, optimizer_local,
                               train_loader, mi_weight, pp_weight, inner_loop)

            if train_loss != train_loss:
                print('NaN')
                continue

            val_acc = eval_acc(model, val_loader, correct_p, commitment_cost, )
        ...
        test_acc = []
        CORRECT_P = [0.55, 0.65, 0.75, 0.85, 0.95]
        for p in CORRECT_P:
            test_acc.append(eval_acc(model, test_loader, p, commitment_cost))
            print('correct_p: {:.3f}, acc: {:.3f}'.
                  format(p, test_acc[-1]))
    ...
    for acc in test_acc_mean:
        print('correct_p:{:.3f}:  acc{:.4f}'.format(cor_p, acc))
        cor_p += 0.1
...
def train(model, correct_p, commitment_cost, discriminator, optimizer, local_optimizer, loader, mi_weight, pp_weight,
          inner_loop):
    model.train()
    ...
    for data in loader:
        ...
        data = data.to(device)
        out, all_pos_embedding, all_graph_embedding, all_pos_penalty = model(data, correct_p, commitment_cost, )
...
def eval_acc(model, loader, correct_p, commitment_cost, ):
    model.eval()
    ...
    for data in loader:
        data = data.to(device)
        with torch.no_grad():
            pred, _, _, _ = model(data, correct_p, commitment_cost, )
            pred = pred.max(1)[1]
        correct += pred.eq(data.y.view(-1)).sum().item()
    return correct / len(loader.dataset)


def eval_loss(model, loader, correct_p, commitment_cost, ):
    model.eval()
    ...
    for data in loader:
        data = data.to(device)
        with torch.no_grad():
            out, _, _, _ = model(data, correct_p, commitment_cost, )
