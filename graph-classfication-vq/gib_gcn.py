...
class GIBGCN(torch.nn.Module):
    ...
    def forward(self, data, correct_p):
        ...
        trans_matrix = self.trans_matrix(correct_p, len(encodings)).to(encodings.device)
        ...
        quantized = all_pos_embedding + (torch.matmul(encodings, codebook) - all_pos_embedding).detach()
