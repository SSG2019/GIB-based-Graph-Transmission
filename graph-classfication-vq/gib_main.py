...
parser.add_argument('--correct_p', type=float, default=0.75)  # BSC正确传输概率
parser.add_argument('--commitment_cost', type=float, default=0.25)  # VQ_VAE 直通估计损失？
    ...
    for num_layers, hidden in product(layers, hiddens):
        ...
        with open(result_path, 'a') as f:
            ...
            for row in ACCs:
                ...
                s = 0.55
                for item in row:
                    f.write("correct_p:{:.3f}, ACC: {:.4f}  ".format(s, item))
                    s += 0.1
                f.write("\n")
            f.write("---MEAN--- \n")
            s = 0.55
            for acc in test_acc:
                f.write("correct_p:{:.3f}, ACC: {:.3f}  ".format(s, acc))
                s += 0.1
            f.write("\n\n")
